...
function FeatureFilm({ type = 'capture' }) {
  ...
  if (type === 'connect') {
    return <div className="feature-film film-connect"><img src="/images/knowledge-connections.webp" alt="Notes and ideas connected around one growing insight" loading="lazy"/><div className="film-connect-shade"/><div className="film-connect-caption">Ideas become a map you can return to.</div></div>;
  }
...
function SolutionsPage() {
  ...
  return <PageWrap className="solutions-page">
    <section className="solutions-hero section-shell"><div><Eyebrow>NXTGEN SOLUTIONS</Eyebrow><h1>Build momentum<br />for every kind of <em>learner.</em></h1><p>Designed for the real places learning happens: inside a study session, across a curriculum, in a team project, or around one shared question.</p><Link className="button button-dark button-large" to="/auth">Explore your space <Arrow /></Link></div><figure className="solutions-signal-map"><img src="/images/learning-journey.webp" alt="A source library, notebook, recall cards, and progress path forming one learning journey"/></figure></section>
...
function PublicLayout({ children }) { const {pathname}=useLocation();return <><Header />{children}{pathname!=='/'&&<QuoteCard />}<Footer /></>; }
